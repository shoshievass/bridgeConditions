...
bridge_spending_by_bridge_and_year <- bridge_spending_by_project_and_bridge %>%
  ...
  group_by(bridgeID, contract_no, start_year) %>%
  summarize(
    num_projects = n()
...
write_csv(bridge_ts, "data/databridge_timeseries_by_proj.csv", na = "NA", col_names = T)

# write.table(bridge_ts, file="data/databridge_timeseries_by_proj.csv", sep=",")

save(bridge_ts, file="data/bridge_timeseries_merged_by_proj.rdata")
