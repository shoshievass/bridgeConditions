...
model {
  ...
  for(i in 1:H){
    ...
    // to_vector(beta_deck[i]) ~ normal(to_vector(beta[i]), to_vector(sigma[i]));
    // to_vector(beta_superstructure[i]) ~ normal(to_vector(beta[i]), to_vector(sigma[i]));
    // to_vector(beta_substructure[i]) ~ normal(to_vector(beta[i]), to_vector(sigma[i]));
  }
