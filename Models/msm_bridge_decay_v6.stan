...
model {
  ...
  for(i in 1:H){
    // to_vector(beta[i]) ~ normal(0, 3);
    // to_vector(sigma[i]) ~ normal(0, 3);
    // to_vector(z_deck[i]) ~ normal(0,3);
    // to_vector(z_superstructure[i]) ~ normal(0,3);
    // to_vector(z_substructure[i]) ~ normal(0,3);
