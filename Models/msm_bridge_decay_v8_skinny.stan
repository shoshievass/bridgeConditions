functions{
  ...
  vector[] forecast_health_state_rng( int N,
                           ...
                           vector[] beta_deck_intercept,
                           vector[] beta_superstructure_intercept,
                           vector[] beta_substructure_intercept,
    ...
    for(b in 1:B){
      ...
      for(t in 1:T_f){
        ...
        if(deck_health_sim_f[n-1] != -999){
          deck_health_sim_f[n] = categorical_rng(softmax(beta_deck_intercept[deck_health_sim_f[n-1]] +  beta_deck[deck_health_sim_f[n-1]] * X_aug_f[n]'));
        }
        else{deck_health_sim_f[n] = -999;}

        if(superstructure_health_sim_f[n-1] != -999){
          superstructure_health_sim_f[n] = categorical_rng(softmax(beta_superstructure_intercept[superstructure_health_sim_f[n-1]] + beta_superstructure[superstructure_health_sim_f[n-1]] * X_aug_f[n]'));
        }
        else{superstructure_health_sim_f[n] = -999;}

        if(substructure_health_sim_f[n-1] != -999){
          substructure_health_sim_f[n] = categorical_rng(softmax(beta_substructure_intercept[substructure_health_sim_f[n-1]] + beta_substructure[substructure_health_sim_f[n-1]] * X_aug_f[n]'));
        }
...
transformed data{
  int deck_num_periods_lapsed[N];
  int n_d;
  int last_new_date;

    for(b in 1:B){

    last_new_date = data_year[ N_b[b] ];
    for(t in 1:(T_b[b] - 1)){
      n_d = N_b[b] + t;
      deck_num_periods_lapsed[n_d] = data_year[n_d] - last_new_date;

      if(deck_health[n_d] != -999 && last_observed_deck_index[n_d] != -999){
          if(deck_health[n_d] != deck_health[last_observed_deck_index[n_d]]){
            last_new_date = data_year[n_d];
          }
      }
    ...
    if(run_estimation==1) {
      ...
      for(t in 1:(t_b-1)){
        ...
        if(deck_health[n] != -999 && last_observed_deck_index[n] != -999){
          ...
          // print("deck beta intercept: ", beta_deck_intercept[deck_health[temp_lag_deck_index]]);
...
generated quantities {
  ...
  for(b in 1:B){
    ...
    for(t in 1:(t_b-1)){
      ...
      if(deck_health[n] != -999 && last_observed_deck_index[n] != -999){
        lpm_deck[n] = log(softmax(beta_deck_intercept[ deck_health[ last_observed_deck_index[n] ] ] + beta_deck[deck_health[ last_observed_deck_index[n] ] ] * X_aug[n]')[deck_health[n]]);
      }
      else{
        lpm_deck[n] = 0;
      }
      if(superstructure_health[n] != -999 && last_observed_superstructure_index[n] != -999){
        lpm_superstructure[n] = log(softmax(beta_superstructure_intercept[superstructure_health[ last_observed_superstructure_index[n] ]] + beta_superstructure[superstructure_health[ last_observed_superstructure_index[n] ] ] * X_aug[n]')[superstructure_health[n]]);
      }
      else{
        lpm_superstructure[n] = 0;
      }
      if(substructure_health[n] != -999 && last_observed_substructure_index[n] != -999){
        lpm_substructure[n] = log(softmax(beta_substructure_intercept[substructure_health[ last_observed_substructure_index[n] ]] + beta_substructure[substructure_health[ last_observed_substructure_index[n] ] ] * X_aug[n]')[substructure_health[n]]);
      }
      else{
  ...
  for(b in 1:B_new){
    ...
    for(t in 1:(t_b-1)){
      ...
      if(deck_health_new[n] != -999 && last_observed_deck_index_new[n] != -999){
        out_of_sample_lpm_deck[n] = log(softmax(beta_deck_intercept[ deck_health_new[ last_observed_deck_index_new[n] ] ] + beta_deck[deck_health_new[ last_observed_deck_index_new[n] ] ] * X_aug_new[n]')[deck_health_new[n]]);
      }
      else{
        out_of_sample_lpm_deck[n] = 0;
      }
      if(superstructure_health_new[n] != -999 && last_observed_superstructure_index_new[n] != -999){
        out_of_sample_lpm_superstructure[n] = log(softmax(beta_superstructure_intercept[superstructure_health_new[ last_observed_superstructure_index_new[n] ]] + beta_superstructure[superstructure_health_new[ last_observed_superstructure_index_new[n] ] ] * X_aug_new[n]')[superstructure_health_new[n]]);
      }
      else{
        out_of_sample_lpm_superstructure[n] = 0;
      }
      if(substructure_health_new[n] != -999 && last_observed_substructure_index_new[n] != -999){
        out_of_sample_lpm_substructure[n] = log(softmax(beta_substructure_intercept[substructure_health_new[ last_observed_substructure_index_new[n] ]] + beta_substructure[substructure_health_new[ last_observed_substructure_index_new[n] ] ] * X_aug_new[n]')[substructure_health_new[n]]);
      }
      else{
